      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          <div className="space-y-2">
            <Badge className="bg-accent text-accent-foreground">Skills</Badge>
        ...
        <div className="mx-auto grid max-w-5xl gap-6 py-12 sm:grid-cols-2 lg:grid-cols-3">
          {skillsData.map((skill) => (
            <Card key={skill.name} className="p-4 transition-all duration-300 hover:shadow-xl hover:scale-105">
              <CardContent className="flex flex-col items-center justify-center p-6 space-y-4">
                {skill.icon}
                <h3 className="text-lg font-bold font-headline">{skill.name}</h3>
                <div className="w-full">
                  <Progress value={skill.level} className="h-3 [&>div]:bg-primary" />
